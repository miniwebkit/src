...
public:
    ...
    PassOwnPtr<SimpleFontData> createScaledFontData(const FontDescription& fontDescription, float scaleFactor) const;
...
#if USE(SKIA) || USE(CG) || USE(CAIRO) || PLATFORM(WX) || USE(SKIA_ON_MAC_CHROME)
...
#if 1 // !(PLATFORM(QT) && !HAVE(QRAWFONT))
